...
class TestGrp261x:
    ...
    def test_device_title(self, browser, metadata, global_mac_addr):
        ...
        * Check Title
        ...
        page.custom_wait(2)
        page.write_requests_log()
        authed_page = Grp261xPageStatusSystemInfo(browser, url=http_ip_addr)
        authed_page.goto("/#page:status_system_info")
        authed_page.write_requests_log()
        authed_page.custom_wait(2)
        assert authed_page.ver_label

        # reboot
        if authed_page.coredump_download_a:
            print("=\tFind Core Dump\t=")
        else:
            print("=\tNo Core Dump\t=")
        authed_page.reboot_btn.click()
        authed_page.custom_wait(2)
        authed_page.popout_ok_btn.click()
